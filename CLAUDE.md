...
- **HTTP** on `:8080` via [chi](https://github.com/go-chi/chi). All routes under `/api` except login require a valid session cookie (`requireAuth`); routes under `/api/admin/*` plus DNS server mutation additionally require `is_admin` (`requireAdmin`). See "Auth & RBAC" below.
  ...
  - `POST /api/dns-servers` — **admin or department-admin**; body requires `isp` (string, mandatory), `address`, and optionally `name`/`protocol`. DNS servers are one shared/global catalog, not department-scoped, so a department admin can add/delete same as a super admin.
