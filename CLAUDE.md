...
--dns-servers dns-server.yaml  # YAML file of DNS servers to query; omit to use system resolver
                               # example file dns-server.yaml is committed to the repo

# Install / sync dependencies (equivalent to pip install -r requirements.txt)
go mod tidy
...
**Concurrency model**: Go goroutines (lightweight threads) communicate via channels (buffered queues). `sync.WaitGroup` acts as a barrier — like `asyncio.gather` or `threading.Barrier` in Python. Context objects carry deadlines and cancellation signals (like `asyncio.timeout` or `threading.Event`).
...
- `AllocatorOptionsWithHostRules(rules)` builds Chrome allocator options with `--host-resolver-rules` so Chrome connects to the pre-resolved IP rather than re-resolving via system DNS. `captureResolved` in `cmd/main.go` creates one Chrome process per compatible group of (URL, IP) pairs — in the common case this is one process total.
...
- Screenshots saved to `<dns_label>/<hostname>/<timestamp>-<urlhash>.png`. DNS label is the server `name` from the YAML (spaces → underscores); falls back to `system` when no DNS servers file is used. The 8-char FNV hash of `url|ip` disambiguates multiple URLs sharing the same hostname, timestamp, or IP.
...
- `dns.Resolve` uses `net.DefaultResolver` (system resolver). `dns.NewResolver(addr)` sends plain UDP queries to a specific server. `dns.NewDoTResolver(addr)` sends queries over TLS (DoT). `dns.NewDoHResolver(endpoint)` sends queries over HTTPS (DoH) using the RFC 8484 GET wire format.
- `--dns-servers` points to a YAML file with a `protocol` field per entry (`udp` default, `dot`, `doh`):
  ```yaml
  servers:
    - name: Cloudflare DoT
      address: 1.1.1.1:853
    ...
    - name: Cloudflare DoH
      ...
      protocol: doh
  ```
- When multiple servers are configured, `runSweep` runs one `pipeline.Run` call per server (DNS-only, noop capture), collects all results, then calls `captureResolved` once. Screenshots are deduped by `(url, resolvedIP)` — same IP across servers shares one screenshot; different IPs get separate screenshots. `assignScreenshots` attaches PNG bytes to the first result per `(url, ip)` pair; others display `(shared)`.
