...
- **IP/ASN + WHOIS enrichment** (`internal/ipinfo/`, `internal/whois/`): informational data points that never affect the compliance verdict, which stays A-record-based.
  ...
  - `ScanResult.ResolvedASN`/`ResolvedOrg`/`ResolvedNetName` — the resolved IP's ASN + network operator via `ipinfo.Fetcher` (`internal/ipinfo/`, ipinfo.io; `--ipinfo-token` for the authenticated tier) plus NetName via a second RDAP IP-network lookup (`whois.FetchIP`, bootstraps to the correct RIR — ARIN/RIPE/APNIC/etc). Both merge into the same `db.IPInfo` row keyed by IP (not domain, since an IP's ASN/NetName don't depend on which domain currently resolves to it) — fetched **at most once per distinct IP, ever** (no periodic refresh). `grpcServer.Submit` (`internal/server/grpc.go`) checks the cache first; on a miss it spawns a **detached goroutine** (`fetchAndCacheIPInfo`, takes both `ipinfo.Fetcher` and `whois.IPFetcher` — nil `netnameFetch` disables the NetName half) so slow/unreachable ipinfo.io or RDAP never blocks scan ingestion, and the two fetches upsert independently so one failing doesn't wipe the other's already-cached field — that scan's row simply has zero ASN/Org/NetName until a later scan re-checks the now-warm cache.
...
- **Screenshot viewing page**: `screenshot_url` is currently only ever surfaced as a raw "View screenshot" link (`results.index.tsx`, `domain.$url.tsx`) that opens the bare PNG in a new tab — there's no in-app gallery for browsing evidence. Likely lands on `domain.$url.tsx` (`/domain/$url`), since that page already lists every scan row (DNS server, timestamp, screenshot) for one URL: add a thumbnail/lightbox view over the existing history table instead of (or alongside) the plain link, so an auditor can flip through a domain's captured screenshots over time without leaving the app.
