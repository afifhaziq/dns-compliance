...
- Routes: `index.tsx` (dashboard with live scan status), `urls.tsx` (URL watchlist management), `dns-servers.tsx` (DNS server management), `results.tsx` (layout route, just an `Outlet`), `results.index.tsx` (`/results` — full compliance results table), `results.$url.tsx` (`/results/$url` — per-URL scan history with a compliance-over-time chart and a per-DNS-server year heatmap, reached via the history icon on the Overview compliance table).
...
- **HTTP** on `:8080` via [chi](https://github.com/go-chi/chi). REST routes all under `/api`:
  ...
  - `GET /api/heatmap/*url` — per-day, per-DNS-server compliance aggregates for a URL (`since`/`until` query params, RFC3339; defaults to the last 7 days), used by the year heatmap on `results.$url.tsx`
