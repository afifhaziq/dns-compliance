...
- **Table pagination**: `/results` (`results.index.tsx`) and `/urls` still render every row client-side with no paging — will degrade as watchlist size grows. (`/results/$url`'s scan history is already paginated, see `results.$url.tsx` above.)
