...
- **Domain status page**: `results.$url.tsx` now shows a `DomainInfoPanel` (registrar/WHOIS via `fetchDomainInfo`) plus per-row resolved IPv6/ASN/org, but this is still the historical scan-by-scan view — there's no dedicated "current status at a glance" summary (latest verdict per DNS server, condensed into one card) separate from the full history table.
- **Breadcrumbs**: no page-to-page navigation trail exists yet — deep pages like `/isps/$isp` and `/results/$url` currently rely on back-button/nav-link navigation only.
- **Table pagination**: the large tables (`/results`, `/urls`) render every row client-side with no paging — will degrade as watchlist/scan-history size grows.
