...
function useAutocompleteContext(componentName: string) {
  ...
  if (!context) throw new Error(`${componentName} must be used inside Autocomplete`);
  return context;
}
...
function composeEventHandlers<Event extends React.SyntheticEvent>(
  ...
  return (event: Event) => {
    ...
    if (!event.defaultPrevented) eventHandler(event);
  };
}
...
function resolvePopupProps(popupProps: DivRenderProps) {
  const {
    ...
    onDrag: _onDrag, onDragEnd: _onDragEnd, onDragEnter: _onDragEnter,
    onDragExit: _onDragExit, onDragLeave: _onDragLeave, onDragOver: _onDragOver,
    onDragStart: _onDragStart, onDrop: _onDrop,
    ref: popupRef, style: popupStyle, ...resolvedPopupProps
  } = popupProps;
  return { popupClassName, popupRef, popupStyle, resolvedPopupProps };
}
...
function resolveItemProps(itemProps: DivRenderProps) {
  const {
    ...
    onDragExit: _onDragExit, onDragLeave: _onDragLeave, onDragOver: _onDragOver,
    onDragStart: _onDragStart, onDrop: _onDrop,
    ref: itemRef, style: itemStyle, ...resolvedItemProps
  } = itemProps;
  return { itemClassName, itemRef, itemStyle, resolvedItemProps };
}

const FLUID_EASE = [0.16, 1, 0.3, 1] as const;
const EXIT_EASE = [0.4, 0, 0.6, 1] as const;

const POPUP_SPRING = { type: "spring" as const, stiffness: 260, damping: 32, mass: 0.95 };
const HIGHLIGHT_SPRING = { type: "spring" as const, stiffness: 460, damping: 40, mass: 0.82 };
...
const popupMotion = {
  ...
  openTransition: {
    ...
    scale: POPUP_SPRING, y: POPUP_SPRING,
  },
  closedTransition: {
...
const autocompleteInputShellClassName =
  "group flex h-11 w-full items-center gap-1 border border-input bg-background pl-3 pr-2 text-base transition-all sm:text-sm hover:border-ring/40 focus-within:border-ring focus-within:ring-1 focus-within:ring-ring/25";
...
const chevronTransition = { type: "spring" as const, stiffness: 380, damping: 30, mass: 0.8 };
...
function Autocomplete<Value>({
  ...
  const internalActionsRef = React.useRef<AutocompletePrimitive.Root.Actions | null>(null);
  ...
  const setOpen = React.useCallback((nextOpen: boolean) => {
    ...
    if (!nextOpen) setActiveValue(undefined);
  }, [isOpenControlled]);
  ...
  >((nextOpen, eventDetails) => {
    ...
    if (!eventDetails.isCanceled) setOpen(nextOpen);
    onOpenChange?.(nextOpen, eventDetails);
  }, [onOpenChange, setOpen]);

  return (
    <AutocompleteContext.Provider value={{
      actionsRef, activeHighlightId, activeValue, open,
      prefersReducedMotion, setActiveValue, setOpen, skipExitAnimationRef,
    }}>
...
function AutocompleteIcon({ className, ...props }: AutocompletePrimitive.Icon.Props) {
  return (
    <AutocompletePrimitive.Icon
      className={cn("pointer-events-none flex shrink-0 items-center text-muted-foreground", className)}
...
function AutocompleteClear({ className, disabled: disabledProp, ...props }: AutocompletePrimitive.Clear.Props) {
  return (
    <AutocompletePrimitive.Clear
      disabled={disabledProp}
      {...props}
      render={(clearProps) => {
        const { className: _c, ref: clearRef, style: _s, ...resolvedClearProps } =
          clearProps as React.ComponentProps<"button"> & { ref?: React.Ref<HTMLButtonElement> };
        return (
          <button
            {...resolvedClearProps}
...
function AutocompleteTrigger({ className, children, disabled = false, ...props }: AutocompletePrimitive.Trigger.Props) {
  const { open, prefersReducedMotion } = useAutocompleteContext("AutocompleteTrigger");
  return (
    ...
    >
      ...
      <motion.span
        ...
        transition={prefersReducedMotion ? INSTANT_TRANSITION : chevronTransition}
      >
...
function AutocompleteInput({
  ...
  const inputGroup = (
    <AutocompletePrimitive.InputGroup
      ...
      onClick={(event) => {
        ...
        if (target.closest('[data-slot="autocomplete-clear"], [data-slot="autocomplete-trigger"]')) return;
    ...
    >
      <AutocompletePrimitive.Input
        ...
        render={(inputProps, inputState) => {
          if (userRender) {
            return typeof userRender === "function" ? userRender(inputProps, inputState) : userRender;
          }
          const { className: primitiveClassName, ref: primitiveRef, style: primitiveStyle, ...resolvedInputProps } =
          ...
          return (
            <InputPrimitive
              ...
              render={(baseInputProps) => {
                const { className: baseClassName, ref: baseRef, style: baseStyle, ...baseResolvedProps } =
                  baseInputProps as InputRenderProps;
                return (
                  <input
                    {...baseResolvedProps}
                    autoComplete="off"
                    className={cn(autocompleteInputClassName, primitiveClassName, baseClassName)}
  ...
  if (!label) return inputGroup;

  return (
    <div className={cn("flex w-full flex-col gap-2", className)}>
      <label className={cn("font-medium text-foreground text-sm", labelClassName)} htmlFor={inputId}>
        {label}
      </label>
      {inputGroup}
    </div>
...
function AutocompleteContentPanel({
  children, className, popupClassName, popupRef, popupState, popupStyle, resolvedPopupProps,
}: {
  children: React.ReactNode;
  className?: string | ((state: AutocompletePrimitive.Popup.State) => string | undefined);
  popupClassName?: string;
  popupRef?: React.Ref<HTMLDivElement>;
  popupState: AutocompletePrimitive.Popup.State;
  popupStyle?: React.CSSProperties;
  resolvedPopupProps: Omit<DivRenderProps, "children" | "className" | "ref" | "style">;
}) {
  ...
  return (
    <div
      {...resolvedPopupProps}
      ref={(node) => { setRef(popupRef, node); }}
      role="presentation"
      style={{ ...popupStyle, transformOrigin: "var(--transform-origin)" }}
    >
      <motion.div
        animate={isPopupVisible ? popupMotion.animate : popupMotion.closed}
        className={cn(
          componentThemeClassName, controlCornerClassName, autocompletePopupPanelClassName,
          popupClassName, resolveStateClassName(className, popupState)
        )}
        initial={popupState.transitionStatus === "starting" ? popupMotion.initial : false}
        onAnimationComplete={() => {
          if (!popupState.open) {
            skipExitAnimationRef.current = false;
        ...
        style={{ pointerEvents: isPopupVisible ? undefined : "none", transformOrigin: "var(--transform-origin)" }}
        transition={
          prefersReducedMotion ? INSTANT_TRANSITION
            : isPopupVisible ? popupMotion.openTransition
            : skipExitAnimation ? INSTANT_TRANSITION
...
function AutocompleteBackdrop({ className, ...props }: AutocompletePrimitive.Backdrop.Props) {
  return (
    <AutocompletePrimitive.Backdrop
      className={cn("fixed inset-0 z-[9998] bg-black/20 dark:bg-black/40", className)}
...
function AutocompleteContent({
  align = "start", alignOffset = 0, anchor, children, className,
  collisionAvoidance = autocompleteCollisionAvoidance,
  collisionPadding = VIEWPORT_PADDING, side = "bottom", sideOffset = 6, ...props
}: AutocompletePrimitive.Popup.Props &
  Pick<AutocompletePrimitive.Positioner.Props, "align" | "alignOffset" | "anchor" | "collisionAvoidance" | "collisionPadding" | "side" | "sideOffset">) {
  return (
    <AutocompletePrimitive.Portal keepMounted>
      <AutocompletePrimitive.Positioner
        align={align} alignOffset={alignOffset} anchor={anchor}
        className="z-[9999] outline-none"
        collisionAvoidance={collisionAvoidance} collisionPadding={collisionPadding}
          ...
          render={(popupProps, popupState) => {
            ...
            return (
              <AutocompleteContentPanel
                className={className} popupClassName={popupClassName}
                popupRef={popupRef} popupState={popupState}
                popupStyle={popupStyle} resolvedPopupProps={resolvedPopupProps}
              >
                {children}
              </AutocompleteContentPanel>
            );
...
function AutocompleteList({ className, onPointerLeave, ...props }: AutocompletePrimitive.List.Props) {
  const { setActiveValue } = useAutocompleteContext("AutocompleteList");
  return (
    <ScrollAreaPrimitive.Root
      className={cn("relative min-h-0 overflow-hidden", className)}
      style={{ maxHeight: `min(var(--available-height, ${MAX_MENU_HEIGHT}px), ${MAX_MENU_HEIGHT}px)` }}
    >
      <AutocompletePrimitive.List
        data-slot="autocomplete-list"
        onPointerLeave={composeEventHandlers(onPointerLeave, () => { setActiveValue(undefined); })}
        {...props}
        render={(listProps) => {
          const { children: listChildren, className: listClassName, ref: listRef, style: listStyle, ...resolvedListProps } =
            listProps as DivRenderProps;
          return (
            <ScrollAreaPrimitive.Viewport
              {...resolvedListProps}
              className={cn("max-h-[inherit] min-h-0 overscroll-contain p-1 outline-none", listClassName)}
              ref={(node) => { setRef(listRef, node); }}
              style={listStyle}
            >
              {listChildren}
            </ScrollAreaPrimitive.Viewport>
      ...
      />
      <ScrollAreaPrimitive.Scrollbar className={autocompleteListScrollbarClassName} orientation="vertical">
...
function AutocompleteItem({
  className, children, description, value, ...props
}: AutocompletePrimitive.Item.Props & { description?: React.ReactNode }) {
  const { activeHighlightId, activeValue, prefersReducedMotion, setActiveValue } =
  ...
  return (
    <AutocompletePrimitive.Item
      ...
      render={(itemProps, itemState) => {
        const { itemClassName, itemRef, itemStyle, resolvedItemProps } = resolveItemProps(itemProps as DivRenderProps);
        ...
        return (
          <motion.div
            ...
            className={cn(
              ...
              controlCornerClassName, itemClassName, resolveStateClassName(className, itemState)
            )}
            initial={prefersReducedMotion ? false : { opacity: 0, y: 2 }}
            onMouseEnter={composeEventHandlers(resolvedItemProps.onMouseEnter, () => { setActiveValue(value); })}
            onPointerMove={composeEventHandlers(resolvedItemProps.onPointerMove, () => { setActiveValue(value); })}
            ref={(node) => { setRef(itemRef, node); }}
            style={itemStyle}
            transition={prefersReducedMotion ? INSTANT_TRANSITION : { duration: 0.12, ease: "easeOut" }}
          >
            {showHighlight ? (
              <motion.div
                className={cn("absolute inset-0 bg-accent", controlCornerInheritClassName)}
  ...
  return (
    ...
    >
      <motion.span
        ...
        transition={prefersReducedMotion ? INSTANT_TRANSITION : { duration: 0.18 }}
      >
        {children}
      </motion.span>
    </AutocompletePrimitive.Empty>
...
function AutocompleteGroup({ className, ...props }: AutocompletePrimitive.Group.Props) {
  return <AutocompletePrimitive.Group className={cn(className)} data-slot="autocomplete-group" {...props} />;
}
