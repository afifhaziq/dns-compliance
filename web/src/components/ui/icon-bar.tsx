...
function IconBar({
  ...
  return (
    <ToggleGroupPrimitive
      ...
      className={cn(componentThemeClassName, "flex flex-wrap items-center gap-2", className)}
      multiple={false}
...
function IconBarItemButton({
  ...
  return (
    <button
      ...
      onMouseEnter={() => { if (!disabled) setHoverPreview(true) }}
      onMouseLeave={() => { if (!disabled) setHoverPreview(false) }}
      ref={mergeRefs}
      type="button"
    >
...
type IconBarItemProps = Omit<TogglePrimitive.Props, "children" | "render" | "value"> & {
  icon: LucideIcon;
  label: string;
  value?: string;
};
    ...
    return (
      <TogglePrimitive
        ...
        render={(renderProps, state) => {
          const { className: renderClassName, ref: renderRef, ...resolvedRenderProps } = renderProps;
          return (
            <IconBarItemButton
              {...resolvedRenderProps}
