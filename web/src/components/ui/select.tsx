...
// Trimmed port of https://www.fluidfunctionalism.com/docs/select — dropped the
// multi-icon-library registry, the pill/rounded shape switcher, and the
// surface-elevation ladder (all orphaned, app-wide keyboard-shortcut-driven
// theming systems this app doesn't use) in favor of this app's existing
// stone/ink design tokens. Kept: proximity hover, spring-animated popover,
// animated checkmark, and the scrollable content with fade cues.
...
const triggerVariants = cva(
  [
    "group inline-flex items-center justify-between gap-2 outline-none cursor-pointer rounded-lg",
  ...
  ],
  {
    variants: {
      variant: {
        bordered:
          "border border-stone-border bg-stone-panel text-foreground hover:border-stone-muted",
        borderless:
          "border border-transparent bg-transparent text-foreground hover:bg-stone-panel",
      },
    },
    defaultVariants: {
      variant: "bordered",
        ...
        <span className="flex items-center gap-2 min-w-0 flex-1">
          {Icon && (
            <Icon
              size={16}
              strokeWidth={1.5}
              className="shrink-0 text-stone-muted transition-colors duration-150 group-hover:text-foreground"
            />
          )}
          <span className="min-w-0 flex-1 text-left truncate">
            {label ?? (
              <span className="text-stone-muted">{placeholder}</span>
            )}
          </span>
        </span>
      ...
      >
        <div
          style={{
            ...
            // while open and only re-enables it (`pointer-events: auto`) on
            // its own DialogContent node. This popover portals to
            // document.body as a sibling, not a descendant of DialogContent,
            // so it inherits the disabled state and becomes invisible to hit
            // testing unless explicitly punched back through here.
            pointerEvents: "auto",
            // Matches b-autocomplete's popover z-index convention for
            // document.body-portalled content — comfortably above any
            // in-app z-50 layer (dialogs, overlays) so this always paints on
            // top regardless of DOM insertion order relative to those layers.
            zIndex: 9999,
          }}
        >
          <motion.div
          ...
          >
            ...
            <AnimatePresence>
              {checkedRect && (
                <motion.div
                  className="absolute rounded-lg bg-ink-subtle pointer-events-none"
                  initial={false}
                  animate={{
                    top: checkedRect.top,
            ...
            <AnimatePresence>
              {activeRect && (
                <motion.div
                  ...
                  className="absolute rounded-lg bg-stone-border/60 pointer-events-none"
                  initial={{
                    opacity: 0,
            ...
            <AnimatePresence>
              {focusRect && (
                <motion.div
                  className="absolute rounded-[10px] pointer-events-none z-20 border-2 border-ink"
                  initial={false}
                  animate={{
  ...
  (
    ...
    return (
      <div
        ...
        className={cn(
          "relative z-10 flex items-center gap-2 rounded-lg px-2 py-2 text-[13px] cursor-pointer outline-none select-none",
...
const SelectLabel = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "px-2 py-1.5 text-[11px] text-stone-muted",
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    role="separator"
    className={cn("my-1 -mx-1 h-px bg-stone-border/60", className)}
    {...props}
  />
));
