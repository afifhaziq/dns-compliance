...
// background-colour gradient fading the content out toward the edge, with a
// small chevron hinting at the scroll direction.
// ---------------------------------------------------------------------------
...
export function useScrollEdges(
  ...
  useEffect(() => {
    ...
    ro.observe(el)
    // Async content (items loading in) changes scrollHeight without resizing
    // the container itself. Coalesce to one update per frame.
    let moRaf = 0
    ...
    >
      <div
        style={{
          position: 'absolute',
          inset: 0,
          background: `linear-gradient(${dir}, transparent 0%, color-mix(in srgb, var(--stone-panel) 75%, transparent) 65%, var(--stone-panel) 100%)`,
        }}
      />
      {chevron && (
        <svg
